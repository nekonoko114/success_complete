...
var finalText = "SUCCESS FIELD"; // 最終的な文字列
var chars = "abcdefghijklmnopqrstuvwxyz"; // ランダムに使用する文字
var duration = 5; // アニメーションの全体時間
...
gsap.to(element, {
  ...
  onUpdate: function onUpdate() {
    var progress = this.progress(); // アニメーションの進行率 (0 ~ 1)
    var currentLength = Math.floor(finalText.length * progress); // 現在の文字数
    var randomText = getRandomText(finalText.length - currentLength); // 残りの文字をランダム化
...
gsap.utils.toArray(".fade-up").forEach(function (expandElem) {
  gsap.fromTo(expandElem, {
    opacity: 0,
    y: 100
  },
  // 初期状態
