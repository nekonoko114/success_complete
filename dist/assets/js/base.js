...
document.addEventListener("DOMContentLoaded", function () {
  var links = document.querySelectorAll('a[href^="#"]'); // IDへのリンクすべてを選択

  links.forEach(function (link) {
    link.addEventListener('click', function (event) {
      event.preventDefault(); // デフォルトのジャンプを無効化

      var targetId = link.getAttribute('href').substring(1); // リンク先ID取得
      var targetElement = document.getElementById(targetId); // 対象要素取得
